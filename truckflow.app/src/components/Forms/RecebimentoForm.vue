    ...
    <v-card class="w-100 rounded-xl" max-width="900" elevation="2">
      ...
      <v-form ref="formRef" @submit.prevent="salvar" class="pa-6">
        ...
        <v-row dense>
          ...
          <v-col cols="12" md="4">
            <v-text-field
              v-model="recebimento.dataInicio"
              type="date"
        ...
        <v-expand-transition>
          <v-card 
            v-if="mostrandoFormItem || recebimento.itensPlanejamento.length === 0" 
            variant="tonal" 
            color="primary" 
        ...
        <div v-if="recebimento.itensPlanejamento.length > 0" class="d-flex flex-column gap-3">
          <v-slide-y-transition group>
            <v-card 
              v-for="(item, index) in recebimento.itensPlanejamento" 
              :key="index"
              elevation="0"
              class="border pa-3 d-flex align-center bg-grey-lighten-5"
            >
              <v-avatar color="blue-grey-lighten-4" size="40" class="mr-3 text-blue-grey-darken-3 font-weight-bold" variant="flat">
                {{ index + 1 }}
              </v-avatar>
              ...
              <v-tooltip text="Remover item" location="top">
                <template v-slot:activator="{ props }">
                    <v-btn v-bind="props" icon="mdi-delete-outline" size="small" variant="text" color="error" @click="removerItem(index)"></v-btn>
                </template>
              </v-tooltip>
            </v-card>
        ...
        <div class="d-flex justify-end mt-8 gap-3">
          ...
          <v-btn
            ...
            size="large"
            type="submit"
            :loading="recebimentoStore.loading"
