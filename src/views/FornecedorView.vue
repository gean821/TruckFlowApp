  ...
  <v-container class="py-10">
    <v-row class="mb-6 align-center">
      <v-col cols="12" sm="8">
        ...
        <p class="text-subtitle-1 text-medium-emphasis">
          Gerencie seus parceiros comerciais e produtos associados
        </p>
      </v-col>
      ...
      <v-col cols="12" md="6">
        ...
        >
          <v-tab value="all" rounded="xl" variant="flat" class="text-none mr-2">
            Todos
            <v-chip size="x-small" class="ml-2 font-weight-bold">{{
              counts.all
            }}</v-chip>
          </v-tab>
          <v-tab
    ...
    <v-row v-else>
      ...
      >
        <v-card
          variant="outlined"
          class="hover-card pb-2 h-100 d-flex flex-column"
          rounded="xl"
        ...
        >
          ...
          <v-card-text class="pt-0 flex-grow-1">
            <div
              class="mb-2 text-caption font-weight-bold text-uppercase text-disabled"
            >
              Produtos Vinculados
            </div>
            <div class="d-flex flex-wrap ga-1">
              <v-chip
                v-for="p in fornecedor.produtos"
                :key="p.id"
                size="x-small"
                color="primary"
                variant="tonal"
                closable
                @click:close="handleUnlink(fornecedor.id!, p.id)"
              >
                {{ p.nome }}
              </v-chip>
              <v-btn
                v-if="!fornecedor.produtos?.length"
                variant="text"
                size="x-small"
                color="grey"
            ...
            <v-tooltip text="Histórico" location="top">
              <template v-slot:activator="{ props }">
                <v-btn
                  ...
                  variant="text"
                  color="grey-darken-1"
                  @click="openHistorico(fornecedor)"
                />
              </template>
            </v-tooltip>
            <v-btn
    ...
    <AuditDrawer
      ...
      :entity-label="
        auditTarget ? `Fornecedor — ${auditTarget.nome}` : undefined
      "
    />
  </v-container>
</template>
...
const editingFornecedor = ref<FornecedorResponse | null>(null);
const targetFornecedor = ref<FornecedorResponse>();
const auditDrawerOpen = ref(false);
const auditTarget = ref<{ id: string; nome: string } | null>(null);
const allProdutos = computed(() => produtos.value);

function openHistorico(fornecedor: FornecedorResponse) {
  if (!fornecedor?.id) {
    return;
  }

  auditTarget.value = { id: fornecedor.id, nome: fornecedor.nome };
...
const filteredFornecedores = computed(() => {
  return fornecedores.value.filter((f) => {
    const matchesTab =
      activeTab.value === "all" ||
      (activeTab.value === "with_products" && f.produtos?.length);
    const query = searchQuery.value?.toLowerCase() || "";
    const matchesSearch =
      f.nome.toLowerCase().includes(query) || f.cnpj.includes(query);
...
const deleteMessage = computed(() =>
  targetFornecedor
    ? `Deseja excluir o fornecedor "${targetFornecedor.value?.nome}"?`
    : "",
);
...
const openConfirmDelete = (f: any) => {
  targetFornecedor.value = f;
  deleteDialogOpen.value = true;
};
const openLinkModal = (f: any) => {
  targetFornecedor.value = f;
...
async function handleConfirmDelete() {
  ...
  try {
    await remove(targetFornecedor.value.id);
